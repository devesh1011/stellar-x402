...
```typescript
{
  scheme: string,                // Payment scheme ("exact" for Stellar)
...
The transaction and signature are separately encoded using Stellar's BCS (Binary Canonical Serialization) format, then base64-encoded for HTTP transport. This structure allows servers to verify payments offline before submitting transactions to the blockchain.
...
Clients can use the transaction hash to verify settlement on the Stellar blockchain explorer, providing an immutable audit trail of all payments.
