# AI IDE Integration Guide for Stellar x402
...
curl -o .cursor/rules/stellar-x402.mdc https://raw.githubusercontent.com/adipundir/stellar-x402/main/integration/stellar-x402.mdc
```
...
```
Set up Stellar x402 payment middleware to protect my existing Next.js API routes:
...
import { paymentMiddleware } from 'stellar-x402'
...
Example: Protect '[YOUR_API_ROUTE]' with price: '1000000', network: 'stellar-testnet'
...
```
Set up Stellar x402 client to consume paid APIs:
...
const response = await wrapFetchWithPayment.get(apiUrl, {
  keypair: process.env.NEXT_PUBLIC_APTOS_PRIVATE_KEY
})
...
Include: Loading state, error handling, Stellar explorer link
```
...
```
I'm integrating Stellar x402, a payment protocol SDK for monetizing APIs with blockchain micropayments.

The SDK provides:
- paymentMiddleware() for protecting Next.js API routes (server-side)
- wrapFetchWithPayment for consuming protected APIs with automatic payment handling (client-side)
- Built on Stellar blockchain with <3s settlement
...
Available at: npm install stellar-x402
Docs: https://stellar-x402.vercel.app/docs
```
...
```
Help me protect my Next.js API routes using Stellar x402 middleware. I want to:
...
```
Show me how to use wrapFetchWithPayment to call a protected API endpoint. The API is at
...
Expected AI Response Pattern:
Your AI should show `wrapFetchWithPayment.get()` or `wrapFetchWithPayment.post()` with privateKey configuration and proper error handling.
...
```
What errors can occur when using Stellar x402 and how should I handle them?
...
```
# Stellar x402 Context

This project uses Stellar x402 for API monetization with blockchain micropayments.
...
- wrapFetchWithPayment.get(url, { privateKey })
- wrapFetchWithPayment.post(url, data, { privateKey })
...
```
@wrapFetchWithPayment Help me create a function that calls multiple protected APIs in
...
// Use public facilitator at https://stellar-x402.vercel.app/api/facilitator
```
...
async function getPremiumWeather(keypair: string): Promise<WeatherAPIResponse> {
  // TODO: Use wrapFetchWithPayment to call https://api.example.com/premium/weather
}
```
...
describe("x402 payment flow", () => {
  it("should automatically handle 402 response and retry with payment", async () => {
    // Test that wrapFetchWithPayment detects 402, signs transaction, and retries
  });
});
```
