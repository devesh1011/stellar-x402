...
The `stellar-x402` SDK provides functions for both **buyers** (consuming paid APIs) and **sellers** (creating paid APIs).

---

## For Buyers (Consuming Paid APIs)

### **Recommended: `wrapFetchWithPayment`**
...
interface AxiosRequestConfig {
  ...
  account?: Keypair; // x402 payment option
}
...
```typescript
import { decodeXPaymentResponse } from "stellar-x402";
...
For advanced use cases, the SDK also exports Stellar utilities:
...
// Get Stellar client
const aptos = getStellarClient("testnet");

// Get account from private key
const account = getKeypairFromPrivateKey("0x...");

// Check balance
const balance = await getKeypairBalance(aptos, accountAddress);
...
- `wrapFetchWithPayment` - Make paid API requests (RECOMMENDED)
...
- `getStellarClient` - Get configured Stellar client
- `getKeypairFromPrivateKey` - Create account from key
- `signAndSubmitPayment` - Sign and submit transactions
- `getKeypairBalance` - Check account balance
...
- `wrapFetchWithPayment` - One function to rule them all!
